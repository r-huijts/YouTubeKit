...
public struct Stream: Sendable {
    ...
    // MARK: - Client Context Properties
    /// The InnerTube client that extracted this stream
    let extractionClient: InnerTube.ClientType?

    /// HTTP headers required for playback of this stream
    ...
    // MARK: - Initializers
    init(format: InnerTube.StreamingData.Format, client: InnerTube.ClientType? = nil) throws {
        ...
        self.extractionClient = client
        self.requiredHeaders = client?.headers ?? [:]

        // Parse mimeType and codecs
        let (parsedMimeType, codecs) = try Extraction.mimeTypeCodec(format.mimeType)
        self.mimeType = parsedMimeType
    ...
    init(remoteStream: RemoteStream) throws {
        ...
        self.itag = itag
        self.videoCodec = remoteStream.videoCodec.flatMap { VideoCodec(rawValue: $0) }
        self.audioCodec = remoteStream.audioCodec.flatMap { AudioCodec(rawValue: $0) }
        ...
        // Remote streams don't have client context (server handles authentication)
        self.extractionClient = nil
        self.requiredHeaders = [:]

        // Backward compatibility for deprecated `type` and `mimeType`
